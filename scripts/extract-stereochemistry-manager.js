...
console.log('Extracting StereochemistryManager...\n');
...
console.log('Step 1: Analyzing stereochemistry methods...');
...
console.log('\nStep 2: Creating StereochemistryManager file...');

const stereochemistryFile = project.createSourceFile('src/StereochemistryManager.ts', '', { overwrite: true });

stereochemistryFile.addImportDeclaration({
  moduleSpecifier: './DrawerBase',
  defaultImport: 'DrawerBase'
});

console.log('Step 3: Creating StereochemistryManager class...');

const stereochemistryClass = stereochemistryFile.addClass({
  name: 'StereochemistryManager',
  isExported: false
});

stereochemistryClass.addProperty({
  name: 'drawer',
  type: 'DrawerBase',
  scope: 'private'
});

stereochemistryClass.addConstructor({
...
for (const methodName of stereochemistryMethods) {
  ...
  if (method) {
    ...
    stereochemistryClass.addMethod({
      name: methodName,
...
stereochemistryFile.addStatements('export = StereochemistryManager;');

console.log('Step 5: Analyzing dependencies...');

const methodBodies = methodsToExtract.map(m => {
  const method = stereochemistryClass.getMethod(m.name);
...
thisReferences.forEach(ref => {
  const isStereochemistryMethod = stereochemistryMethods.includes(ref);
  const marker = isStereochemistryMethod ? '[METHOD]' : '[EXTERNAL]';
  console.log(`  ${marker} this.${ref}`);
});

console.log('\nStep 6: Saving StereochemistryManager.ts...');
stereochemistryFile.saveSync();
console.log(`Saved: ${stereochemistryFile.getFilePath()}`);
...
console.log('2. Update DrawerBase.ts to use StereochemistryManager');
