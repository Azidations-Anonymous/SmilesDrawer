...
- SVG label rendering now defaults to the Pikachu-style absolute layout: every glyph gets its own `<text>` node (no `<g>` transforms), mask halos follow the rendered anchor, and a new `svgTextParity` option allows falling back to the legacy block renderer if needed.
