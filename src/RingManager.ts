...
class RingManager {
        ...
        let recurse = function (r) {
          ...
          for (var i = 0; i < ring.neighbours.length; i++) {
            ...
            if (involvedRings.indexOf(n) === -1 &&
              n !== r &&
              RingConnection.isBridge(that.ringConnections, that.drawer.graph.vertices, r, n)) {
              recurse(n);
            }
          }
        };
    ...
    isPartOfBridgedRing(ringId: number): boolean {
        for (var i = 0; i < this.ringConnections.length; i++) {
          if (this.ringConnections[i].containsRing(ringId) &&
            this.ringConnections[i].isBridge(this.drawer.graph.vertices)) {
            return true;
          }
        }
    ...
    createBridgedRing(ringIds: number[], sourceVertexId: number): any {
        ...
        for (let id of vertices) {
          let vertex = this.drawer.graph.vertices[id as number];
