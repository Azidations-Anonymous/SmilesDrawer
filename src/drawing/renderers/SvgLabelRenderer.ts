...
class SvgLabelRenderer {
  constructor(private userOpts: IUserOptions, private getBackgroundColor: () => string) {}
  ...
  drawPrimaryLabel(x: number, y: number, text: string, color: string, fontSize?: number): SVGElement {
    const fallbackSize = this.userOpts.typography.fontSizeLarge;
    return this.createText(x, y, text, color, fontSize || fallbackSize, 'primary');
  }

  drawSatellite(x: number, y: number, text: string, color: string, fontSize?: number, category?: LabelCategory): SVGElement {
    const fallbackSize = this.userOpts.typography.fontSizeLarge;
    return this.createText(x, y, text, color, fontSize || fallbackSize, 'satellite', category);
  }
  ...
  private createText(x: number, y: number, text: string, color: string, fontSize: number, role: LabelRole, category?: LabelCategory): SVGElement {
    ...
    const outlineWidth = this.userOpts.typography.labelOutlineWidth ?? 0;
    ...
    textElem.setAttributeNS(null, 'font-family', this.userOpts.typography.fontFamily);
    ...
    tspan.setAttributeNS(null, 'dy', `${this.userOpts.typography.svgBaselineShiftEm}em`);
